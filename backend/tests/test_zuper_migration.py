...
Every pre-existing table keeps its columns (opportunities gains exactly its three lead outcome
columns, at the end) and every row; the added columns read NULL; exactly the eight zuper_*
tables appear, empty; their server defaults fill a row inserted without them; the round trip is
clean; and `upgrade()` calls nothing but create_table, create_index and add_column.
"""
import ast
import pathlib
...
NEW_TABLES = {"zuper_settings", "zuper_mappings", "zuper_sync_state", "zuper_webhook_inbox",
              ...
              "zuper_digests"}
ADDED = {"opportunities": ["lead_outcome", "lead_outcome_note", "lead_outcome_set_at"]}
...
def test_the_migration_only_adds_and_keeps_every_row():
    ...
    for table, cols in before_cols.items():
        ...
        assert all(c[2] for c in after_cols[table][len(cols):]), "an added column is NOT NULL"
    for table, rows in before_rows.items():
        ...
        for old, new in zip(rows, after_rows[table], strict=True):
            ...
            assert all(new[k] is None for k in ADDED.get(table, [])), table
    ...
    with _database_at(REVISION) as eng:
        with eng.begin() as conn:
            ...
            assert tuple(conn.execute(text(
                "SELECT lead_outcome, lead_outcome_note, lead_outcome_set_at FROM opportunities"
            )).one()) == (None, None, None)
            ...
            assert conn.execute(text(
                "SELECT removed_in_zuper FROM zuper_documents")).scalar() == 0
        command.downgrade(_config(), PREVIOUS_HEAD)
        eng.dispose()
        with eng.connect() as conn:
            assert not NEW_TABLES & set(inspect(conn).get_table_names())
            assert "lead_outcome" not in [c["name"] for c in
                                          inspect(conn).get_columns("opportunities")]
