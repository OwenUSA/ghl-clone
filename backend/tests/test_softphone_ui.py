...
def test_no_page_owns_the_softphone():
    """feature/conversations-live is rewriting ConversationsPage. The softphone must not
    appear there, or the two branches collide over a file neither of them needs to share."""
    for page in sorted((FRONTEND / "pages").glob("*.tsx")):
        source = page.read_text(encoding="utf-8")
        assert "softphone" not in source.lower(), (
            f"{page.name} references the softphone — it belongs in the app shell, "
            "not in a page")
